# BOL-005popAdj-PWTscaled / ADM1 — ADM1 (Bolivia)
...
PWT-anchored local GDP for **Bolivia** at the **ADM1** level, 2012–2022 — the
0.25-degree gridded estimates **rescaled so national totals equal Penn World Table 11.0**, in
interpretable 2021 PPP US$. This dataset holds **9 level-1 regions × 11 years = 99 rows**.
...
- `bolivia_adm1.csv` — the panel as text (one row per observation)
- `bolivia_adm1.dta` — the panel as a labelled Stata 118 file
- `bolivia_adm1.gpkg` — boundaries + per-year `gdppc_<year>` columns (for choropleths)
- `bolivia_adm1.parquet` — the panel (fast columnar load)
- `bolivia_adm1_boundaries.gpkg` — the region boundary polygons (EPSG:4326)
- `bolivia_adm1_data_def.csv` — the machine-readable column dictionary (expdpy `df_def`)
...
The same dictionary is in `bolivia_adm1_data_def.csv` (expdpy `df_def`: `var_name, var_def, label, type, role, can_be_na`).
